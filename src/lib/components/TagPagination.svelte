<script lang="ts">
    ...
    let { currentPage, totalPages, totalArticles, tagSlug }: TagPaginationProps = $props();
  ...
  {#if totalPages > 1}
    <nav class="flex items-center justify-center mt-16 mb-8" aria-label="Tag Articles Pagination">
      <div class="flex items-center gap-2">
        <!-- Previous Button -->
        {#if hasPrevious}
          <a
            href="/tags/{tagSlug}{currentPage - 1 === 1 ? '' : `?page=${currentPage - 1}`}"
        ...
        <div class="flex items-center gap-1">
          {#each pageNumbers as pageNum}
            ...
            {:else}
              ...
              <a
                href="/tags/{tagSlug}{pageNum === 1 ? '' : `?page=${pageNum}`}"
                data-sveltekit-preload-data="hover"
