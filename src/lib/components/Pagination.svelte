<script lang="ts">
    ...
    let { currentPage, totalPages, totalArticles }: PaginationProps = $props();
  ...
  {#if totalPages > 1}
    <nav class="flex items-center justify-center mt-16 mb-8" aria-label="Pagination">
      <div class="flex items-center gap-2">
        ...
        {#if hasPrevious}
          <a
            href="/reviews{currentPage - 1 === 1 ? '' : `?page=${currentPage - 1}`}"
        ...
        <div class="flex items-center gap-1">
          {#each pageNumbers as pageNum}
            ...
            {:else}
              ...
              <a
                href="/reviews{pageNum === 1 ? '' : `?page=${pageNum}`}"
                data-sveltekit-preload-data="hover"
    ...
    <div class="text-center text-sm text-text-light mb-8">
      Showing {((currentPage - 1) * 12) + 1}-{Math.min(currentPage * 12, totalArticles)} of {totalArticles} reviews
    </div>
  {/if}